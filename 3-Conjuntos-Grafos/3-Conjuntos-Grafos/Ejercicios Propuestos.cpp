...
// Nivel básico
	gmap :: iterator g = G.begin();
	while( g != G.end()){
		if( (g->second.size()-1) != G.size())
			return false;
		g++;
	}
	return true;
...
int cantAristasDirigido(graph &G){
	graph :: iterator g = G.begin();
	int cont = 0;
	while ( g != G.end()){
		set<int> :: iterator s = g->begin();
		while( s != g->end() ){
			cont++;
			s++;
		}
...
int cantaristasDG(gmap &G){
	gmap::iterator g=G.begin();
	int cont=0;
	while(g!=G.end()){
		set<int> &aux=g->second;
		set<int>::iterator p=aux.begin();
		while (p!=aux.end()){
			cont++;
			p++;
		}
...
bool is_eulerian_path(gmap &G, list<int> &L){
	...
	while ( l != L.end()){
		...
		if( aux != L.end()){
			...
			if (g->second.find(*aux) == g->second.end() ){
				return false;
			}
		}
		l++;
	}
	//Falta contemplar el caso de que un nodo se visite dos veces que se haria guardando en un vector los visitados;
	return true;
}
...
int main(int argc, char *argv[]) {
	...
	cout << es_completo(G) << endl;
	cout << cantAristasDirigido(GG) << endl;
	cout << cantaristasDG(G) << endl;
	cout << cantAristasNoDirigido(GG) << endl;
	cout << is_eulerian_path(G, L);
	return 0;
}
