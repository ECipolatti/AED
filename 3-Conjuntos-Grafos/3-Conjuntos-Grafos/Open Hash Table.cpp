...
void oht_resize(vector<list<int> > &table , unsigned int (*hash) (const int&),
				const int &value){
	
	vector<list<string> > newtable(newsize);
	for( int b=0; b<table.size(); b++){
		
		list<string> &L = table[b];
		list<string> :: iterator l = L.begin(), posotion;
		
		whie( p != L.end()){
			int bucket;
			oht_insert( newtable,hash,*p,bucket,position);
			p++;
		}
